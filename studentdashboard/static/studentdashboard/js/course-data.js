/* =========================================================================
   ...
   -------------------------------------------------------------------------
   This file is the single source of truth for the whole "Continue" flow:

     Dashboard → Continue clicked → Read course ID → Load saved progress →
     Find current lesson → Open lesson.html → Generate lecture page →
     Watch video → Take quiz → Complete assignment → Mark lesson complete →
     Update progress → Open next lesson → Repeat until 100% →
...
(function (global) {
  ...
  /* ----------------------------------------------------------------------
     3. PROGRESS STORAGE (localStorage)
     ---------------------------------------------------------------------- */
  ...
  // First time a course is touched, seed its progress from the raw
  // `completed:` defaults authored above. After that, localStorage wins.
  function ensureCourseSeeded(courseId) {
    const store = getProgressStore();
    if (!store[courseId]) {
      const raw = RAW_COURSES[courseId];
      const seeded = raw ? raw.lessons.filter(l => l.completed).map(l => l.id) : [];
      store[courseId] = { completedLessonIds: seeded };
  ...
  function getCertificateStore() {
    return readJSON(CERT_KEY, {});
  }
  function saveCertificateStore(store) {
    writeJSON(CERT_KEY, store);
  }
  ...
  function getCourse(courseId) {
    const raw = RAW_COURSES[courseId];
    if (!raw) return null;
    const store = ensureCourseSeeded(courseId);
  ...
  function getCurrentLesson(courseId) {
    const course = getCourse(courseId);
  ...
  function setLessonComplete(courseId, lessonId, value) {
    ensureCourseSeeded(courseId);
    const store = getProgressStore();
  ...
  // Generate certificate automatically once a course hits 100%.
  ...
  function getCertificate(courseId) {
    return getCertificateStore()[courseId] || null;
  }
  ...
  // progress instead of hardcoded mockup values.
  function getOverallStats() {
    const ids = listCourseIds();
    let inProgress = 0, totalPct = 0, certs = 0, completedMinutes = 0;
    ids.forEach(id => {
      const course = getCourse(id);
      const pct = getCourseCompletionPercent(id);
      totalPct += pct;
      if (pct < 100) inProgress++;
      if (getCertificate(id)) certs++;
  ...
  global.getRawCourse = function (courseId) { return RAW_COURSES[courseId] || null; };
